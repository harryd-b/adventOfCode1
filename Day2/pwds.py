...
def check_all(passwords_list, part):
    ...
    for i in range(len(passwords_list)):
        check_list = split_data(passwords_list[i])
        if part == 1:
            counter += check_pwd(check_list[0], check_list[1], check_list[2], check_list[3])
        else:
            counter += check_pwd_1(check_list[0], check_list[1], check_list[2], check_list[3])
    return counter
